...
public:
	...
	void Ke(const vector<vector<double>>& ElementNodeCoordinate,vector<vector<double>>& Ke) {
		...
		// 在自然坐标原点处计算 NDerivative0 和 JacobiDET0，用于计算 BdilBar
		vector<double> N0;
		vector<vector<double>> NDerivative0;
		double JacobiDET0;
		ShapeFunction({ 0.0, 0.0, 0.0 }, ElementNodeCoordinate, N0, NDerivative0, JacobiDET0);
		...
		for (int X = 0; X < 2; ++X) {
			for (int Y = 0; Y < 2; ++Y) {
				for (int Z = 0; Z < 2; ++Z) {
					...
					// 初始化 Bdil 为 6x24 的零矩阵
					vector<vector<double>> Bdil(6, vector<double>(24, 0.0));
					// 构建 Bdil 矩阵
					for (int I = 0; I < 8; ++I) {
						int COL = I * 3; // 3代表xyz

						// 填充 Bdil 矩阵对应的列
						Bdil[0][COL] = NDerivative[0][I];
						Bdil[1][COL] = NDerivative[0][I];
						Bdil[2][COL] = NDerivative[0][I];
						Bdil[0][COL + 1] = NDerivative[1][I];
						Bdil[1][COL + 1] = NDerivative[1][I];
						Bdil[2][COL + 1] = NDerivative[1][I];
						Bdil[0][COL + 2] = NDerivative[2][I];
						Bdil[1][COL + 2] = NDerivative[2][I];
						Bdil[2][COL + 2] = NDerivative[2][I];
					...
					// 计算 BdilBar 矩阵（在自然坐标系原点处）
					vector<vector<double>> BdilBar(6, vector<double>(24, 0.0));
					for (int I = 0; I < 8; ++I) {
						int COL = I * 3; // 3代表xyz

						// 填充 Bdil 矩阵对应的列
						BdilBar[0][COL] = NDerivative0[0][I];
						BdilBar[1][COL] = NDerivative0[0][I];
						BdilBar[2][COL] = NDerivative0[0][I];
						BdilBar[0][COL + 1] = NDerivative0[1][I];
						BdilBar[1][COL + 1] = NDerivative0[1][I];
						BdilBar[2][COL + 1] = NDerivative0[1][I];
						BdilBar[0][COL + 2] = NDerivative0[2][I];
						BdilBar[1][COL + 2] = NDerivative0[2][I];
						BdilBar[2][COL + 2] = NDerivative0[2][I];
					...
					// 计算 Bdev 和 Bbar 矩阵
					vector<vector<double>> Bdev(6, vector<double>(24, 0.0));
					for (int i = 0; i < 6; ++i) {
						for (int j = 0; j < 24; ++j) {
							Bdev[i][j] = B[i][j] - Bdil[i][j];
						}
					}

					vector<vector<double>> Bbar(6, vector<double>(24, 0.0));
					for (int i = 0; i < 6; ++i) {
						for (int j = 0; j < 24; ++j) {
							Bbar[i][j] = Bdev[i][j] + BdilBar[i][j];
						}
					}

					// 计算 temp = D（6*6） * Bbar（6*24）
					vector<vector<double>> temp(6, vector<double>(24, 0.0));
					for (int i = 0; i < 6; ++i) {
						for (int j = 0; j < 24; ++j) {
							for (int k = 0; k < 6; ++k) {
								temp[i][j] += D(i,k) * Bbar[k][j];
							}
						}
					}

					// 计算 Ke 增量：Ke_increment = Coefficient * Bbar^T * temp（24*24）
					vector<vector<double>> Ke_increment(24, vector<double>(24, 0.0));
					for (int i = 0; i < 24; ++i) {
						for (int j = 0; j < 24; ++j) {
							for (int k = 0; k < 6; ++k) {
								Ke_increment[i][j] += Bbar[k][i]/* 转置 */ * temp[k][j];
							}
...
private:
	double E;    // 弹性模量，单位 Pa
	double u;    // 泊松比，无量纲
	Eigen::Matrix<double, 6, 6> D;   // 应力应变矩阵 D
};